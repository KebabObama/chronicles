...
export const Button = ({ className, type = "button", children, sub, ...props }: ButtonProps) => {
  return (
    <div
      ...
      {sub && (
        <div className="absolute hidden md:block top-full left-1/2 -translate-x-1/2 transition-all duration-500 z-10 ease-out bg-background px-3 py-2 group-hover/button:opacity-100 group-hover/button:translate-y-1 group-hover/button:pointer-events-auto opacity-0 translate-y-2 pointer-events-none rounded-bl-2xl rounded-tr-2xl border-2">
          {sub}
        </div>
      )}
      <div
