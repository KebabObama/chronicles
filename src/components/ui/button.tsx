...
export const Button = ({ className, type = "button", children, sub, ...props }: ButtonProps) => {
  return (
    <div
      ...
      <svg
        className="size-2 group-hover/button:size-3 absolute top-[calc(100%-0.25rem)] z-5 left-1/2 -translate-x-1/2 transition-all duration-300 group-hover/button:text-red-400 group-hover/button:translate-y-0.5 ease-out"
        viewBox="0 0 12 11"
        fill="none"
      ...
      {sub ? (
        <div
          className={cn(
            "absolute top-full left-1/2 -translate-x-1/2 transition-all duration-300 ease-out bg-card px-3 py-2",
